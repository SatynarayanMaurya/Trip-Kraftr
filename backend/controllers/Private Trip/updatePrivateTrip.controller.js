...
export const updatePrivateTripStatus = async (req, res) => {
    try {
        const { privateTripId } = req.params;
        ...
        if (!privateTripId) {
            return res.status(400).json({
                success: false,
                message: "Private Trip id not found"
            });
        }
        ...
        const updatedPrivateTrip = await PrivateTrip.findOneAndUpdate(
            {
                org_id: req.user.org_id,
                _id: privateTripId
            },
        ...
        return res.status(200).json({
            success: true,
            message: "Private Trip status updated successfully",
            data: updatedPrivateTrip
        });
