...
function UpdateActivity() {
    const { getActivityById, updateActivityById } = useActivityHooks();
    ...
    useEffect(() => {
        ...
        setFormData((prev) => ({
            ...prev,
            regionId: activityDetails?.regionId?._id || "",
            region: activityDetails?.regionId?.name || "",
            subRegionId: activityDetails?.subRegionId?._id || "",
            subRegion: activityDetails?.subRegionId?.name || "",
            ...
            existingImageUrl: activityDetails?.imageUrl || null,
            preview: activityDetails?.imageUrl || null,
        }));
    }, [activityDetails]);
    ...
    const validate = () => {
        ...
        if (!formData.activityName) newErrors.activity = "Activity name is required";
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const response = await updateActivityById(payload)
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            />
                            {errors.activityName && <p className="mt-1 text-xs text-red-500">{errors.activityName}</p>}
                        </div>
