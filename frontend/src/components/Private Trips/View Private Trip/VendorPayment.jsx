    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            {vehiclePayments.length > 0 && (
                <section>
                    <SectionHeader icon={Car} iconBg="bg-pink-100" iconColor="text-[#ED5F8D]" title="Vehicle Vendors" count={vehiclePayments.length} />
                    {vehiclePayments.map((vehicle, i) => {
                        const key = `vehicle-${vehicle.vehicleId?._id ?? i}`
                        return (
                            <CollapsibleSection
                                key={key}
                                data={vehicle}
