...
import { blankDay } from '../Add Private Trip/HotelDetailsPrivateTrip';
...
function EditPrivateTrips() {
  ...
  const { addPrivateTrip, getPrivateTripById, updatePrivateTripById } = usePrivateTripHooks()
  ...
  useEffect(() => {
    if (privateTripDetails) {

      setFormData(prev => ({
        ...prev,
        regionDetails: {
          region1: privateTripDetails?.regionDetails?.region1?._id || privateTripDetails?.regionDetails?.region1 || null,
          region2: privateTripDetails?.regionDetails?.region2?._id || privateTripDetails?.regionDetails?.region2 || null,
          region3: privateTripDetails?.regionDetails?.region3?._id || privateTripDetails?.regionDetails?.region3 || null,
          startDate: convertUTCToISTDate(privateTripDetails?.regionDetails?.startDate) || '',
          noOfDays: privateTripDetails?.regionDetails?.noOfDays || '',
          adults: privateTripDetails?.regionDetails?.adults || 0,
          children: privateTripDetails?.regionDetails?.children || 0,
          childAges: privateTripDetails?.regionDetails?.childAges || [],
        },


        itineraryBuilder: {
          tripName: privateTripDetails?.itineraryBuilder?.tripName || '',
          tripOverview: privateTripDetails?.itineraryBuilder?.tripOverview || '',

          daysDetails: (privateTripDetails?.itineraryBuilder?.daysDetails || []).map(day => ({
            ...day,
      ...
      setEnquiryDetails(privateTripDetails?.enquiryId)
      setSearchEnquiry(prev => ({
        ...prev,
        search: privateTripDetails?.enquiryId?.accountId?.fullName || privateTripDetails?.enquiryId?.accountId?.businessName || '',
        enquiryType: privateTripDetails?.enquiryType || 'b2c'
      }))
      ...
      setCustomerDetails(privateTripDetails?.enquiryId?.accountId)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '24px', background: '#f5f6fa', minHeight: '100vh' }}>
      <h1 style={{ fontSize: '22px', fontWeight: '700', color: BLUE, margin: 0 }}>Update Private Trip</h1>
      <button
        onClick={() => navigate(-1)}
...
export default EditPrivateTrips